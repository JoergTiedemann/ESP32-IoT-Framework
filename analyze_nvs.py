# Aufruf python analyze_nvs.py nvs.bin -s -b 32
import argparse
import os
import sys
...
# Keys used for passw
# These are stored as BLOBs but can be printed as strings
PSW_KEY_VALUES = ["sta.pswd", "ap.passwd"]
...
def scan_in_page(page_data, page_index, ns_only):
    ...
    while i < len(entry_states):
        ...
        if entry_states[i] == EntryState.WRITTEN:
            ...
            if entry_type < 0x20:
                ...
                # Don't try to parse other types
                pass
            ...
            elif not (ns_only or do_not_add):
                namespace_name = ns_idx_to_name[entry_ns]
...
def parse_pages(input, ns_only):
    for page in page_array:
        page_idx = page[0]
        seq_no = page[1]
        # print("Parsing page %d with seq no %d" % (page_idx, seq_no))
        page_base = page_idx * PAGE_SIZE
        scan_in_page(input[page_base:page_base+PAGE_SIZE], page_idx, ns_only)

def parse_nvs_binary(input):
    build_page_array(input)
    parse_pages(input, True)
    parse_pages(input, False)
...
def entry_data_to_string(entry_key, entry_type, entry_data):
    ...
    if entry_type < 0x20:
        ...
        if special_handling:
            ...
            handle_passphrase = True if entry_key in PSW_KEY_VALUES else False
        else:
            handle_ssid = False
