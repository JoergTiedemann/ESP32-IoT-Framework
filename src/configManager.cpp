...
void config::setConfigSaveCallback(xSemaphoreHandle* pMutexLock, std::function<void()> func ) {
  _configsavecallback = func;
  m_pMutexLock = pMutexLock;
}
...
void config::loop()
{
    if (requestSave)
    {
        ...
        if (m_pMutexLock)
        {
            if (xSemaphoreTake(*m_pMutexLock, 200000 / portTICK_PERIOD_MS))
            {
                save();
                xSemaphoreGive(*m_pMutexLock);
            }
        }
        else
            save();
