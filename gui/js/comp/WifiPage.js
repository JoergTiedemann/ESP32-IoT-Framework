...
export function WifiPage(props) {
    ...
    // console.log("WifiPage: state.wiFiInfo",state.wiFiInfo);
    const wifiInfo = Array.isArray(state.wiFiInfo)
        ? state.wiFiInfo
        : (Array.isArray(state.wifiInfo) ? state.wifiInfo : []);

    if (wifiInfo != null && wifiInfo.length > 0) {
        for (let i = 0; i < wifiInfo.length; i++) {
            page = <>{page}<p>{wifiInfo[i]}</p></>;
        }
    }
